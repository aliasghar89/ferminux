...
func makeHeaderChain(parent *types.Header, n int, db fmxdb.Database, seed int) []*types.Header {
	blocks, _ := core.GenerateChain(params.TestChainConfig, types.NewBlockWithHeader(parent), powhash.NewFaker(), db, n, func(i int, b *core.BlockGen) {
...
// header only chain.
func newCanonical(n int) (fmxdb.Database, *LightChain, error) {
	db := rawdb.NewMemoryDatabase()
	gspec := core.Genesis{Config: params.TestChainConfig}
	genesis := gspec.MustCommit(db)
	blockchain, _ := NewLightChain(&dummyOdr{db: db, indexerConfig: TestClientIndexerConfig}, gspec.Config, powhash.NewFaker(), nil)
...
func newTestLightChain() *LightChain {
	...
	gspec.MustCommit(db)
	lc, err := NewLightChain(&dummyOdr{db: db}, gspec.Config, powhash.NewFullFaker(), nil)
	if err != nil {
		panic(err)
	}
	return lc
...
func (odr *dummyOdr) Database() fmxdb.Database {
	return odr.db
}
...
func TestReorgBadHeaderHashes(t *testing.T) {
	...
	ncm, err := NewLightChain(&dummyOdr{db: bc.chainDb}, params.TestChainConfig, powhash.NewFaker(), nil)
	if err != nil {
