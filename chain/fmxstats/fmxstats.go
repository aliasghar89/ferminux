...
import (
	...
	fmxproto "github.com/aliasghar89/ferminux/chain/fmx/protocols/fmxproto"
...
type backend interface {
	...
	SyncProgress() ferminux.SyncProgress
}
...
// Service implements a Ferminux netstats reporting daemon that pushes local
...
func (s *Service) login(conn *connWrapper) error {
	...
	if info := infos.Protocols["eth"]; info != nil {
		network = fmt.Sprintf("%d", info.(*fmxproto.NodeInfo).Network)
	} else {
