...
import (
	...
	"github.com/aliasghar89/ferminux/chain/signer/storage"
)
...
type ExternalAPI interface {
	...
	SignTransaction(ctx context.Context, args apitypes.SendTxArgs, methodSelector *string) (*fmxapi.SignTransactionResult, error)
...
type UIClientAPI interface {
	...
	OnApprovedTx(tx fmxapi.SignTransactionResult)
...
func (api *SignerAPI) SignTransaction(ctx context.Context, args apitypes.SendTxArgs, methodSelector *string) (*fmxapi.SignTransactionResult, error) {
	var (
		err    error
	...
	response := fmxapi.SignTransactionResult{Raw: data, Tx: signedTx}
