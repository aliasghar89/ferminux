...
func ReadDatabaseVersion(db fmxdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db fmxdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db fmxdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db fmxdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
// given genesis hash.
func ReadGenesisStateSpec(db fmxdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(genesisStateSpecKey(hash))
	return data
}

// WriteGenesisStateSpec writes the genesis state specification into the disk.
func WriteGenesisStateSpec(db fmxdb.KeyValueWriter, hash common.Hash, data []byte) {
...
func PushUncleanShutdownMarker(db fmxdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db fmxdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func UpdateUncleanShutdownMarker(db fmxdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)
...
func ReadTransitionStatus(db fmxdb.KeyValueReader) []byte {
	data, _ := db.Get(transitionStatusKey)
	return data
}

// WriteTransitionStatus stores the eth2 transition status to the database
func WriteTransitionStatus(db fmxdb.KeyValueWriter, data []byte) {
