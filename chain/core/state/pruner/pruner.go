...
func NewPruner(db fmxdb.Database, datadir, trieCachePath string, bloomSize uint64) (*Pruner, error) {
...
func prune(snaptree *snapshot.Tree, root common.Hash, maindb fmxdb.Database, stateBloom *stateBloom, bloomPath string, middleStateRoots map[common.Hash]struct{}, start time.Time) error {
...
// in the disk.
func RecoverPruning(datadir string, db fmxdb.Database, trieCachePath string) error {
...
func extractGenesis(db fmxdb.Database, stateBloom *stateBloom) error {
...
pruning. Remember don't start the Ferminux without deleting the clean trie cache
otherwise the entire database may be damaged!

Check the command description "ferminux snapshot prune-state --help" for more details.
`
