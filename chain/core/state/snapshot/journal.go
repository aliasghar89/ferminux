...
func loadAndParseJournal(db fmxdb.KeyValueStore, base *diskLayer) (snapshot, journalGenerator, error) {
	...
	// not existent, e.g. the disk layer is generating while that the Ferminux
...
func loadSnapshot(diskdb fmxdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash, recovery bool) (snapshot, bool, error) {
	...
	//
	// Possible scenario: Ferminux was crashed without persisting journal and then
...
func iterateJournal(db fmxdb.KeyValueReader, callback journalCallback) error {
