...
type ChainIndexer struct {
	chainDb  fmxdb.Database      // Chain database to index the data from
	indexDb  fmxdb.Database      // Prefixed table-view of the db to write index metadata into
...
func NewChainIndexer(chainDb fmxdb.Database, indexDb fmxdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
