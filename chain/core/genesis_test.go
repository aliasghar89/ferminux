...
func TestSetupGenesis(t *testing.T) {
	...
	tests := []struct {
		name       string
		fn         func(fmxdb.Database) (*params.ChainConfig, common.Hash, error)
	...
	}{
		{
			...
			fn: func(db fmxdb.Database) (*params.ChainConfig, common.Hash, error) {
				return SetupGenesisBlock(db, new(Genesis))
			},
			wantErr:    errGenesisNoConfig,
			wantConfig: params.AllPowhashProtocolChanges,
		},
		{
			...
			// Ferminux fork: the default network is Ferminux, not mainnet.
			wantHash:   params.FerminuxGenesisHash,
			wantConfig: params.FerminuxChainConfig,
		},
		{
		...
		},
		{
			...
			fn: func(db fmxdb.Database) (*params.ChainConfig, common.Hash, error) {
				...
				bc, _ := NewBlockChain(db, nil, oldcustomg.Config, powhash.NewFullFaker(), vm.Config{}, nil, nil)
				defer bc.Stop()

				blocks, _ := GenerateChain(oldcustomg.Config, genesis, powhash.NewFaker(), db, 4, nil)
