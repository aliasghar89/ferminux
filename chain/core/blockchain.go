...
type CacheConfig struct {
	...
	// FerminuxMaxReorgDepth caps chain reorganisations: while the chain head
	// is an authority block (ChainConfig.IsPosa) a reorg that would drop more
	// than this many canonical blocks is refused. 0 disables the cap. The
	// node wiring (eth/backend.go, cmd/utils.MakeChain) sets it to
	// params.FerminuxMaxReorgDepth unless the node was started with
	// --ferminux.allowdeepreorg; library users and tests passing a nil
	// CacheConfig get no cap. With PosaBlock unset the cap never engages.
	FerminuxMaxReorgDepth int
...
func (bc *BlockChain) reorg(oldBlock, newBlock *types.Block) error {
	...
	// Ferminux reorg-depth cap: while the head is an authority block (number
	// >= PosaBlock) refuse to drop more than FerminuxMaxReorgDepth canonical
	// blocks unless the operator explicitly allowed it. This is the only thing
	// standing between a signer-majority history rewrite and unbacked wFMX on
	// BSC: the bridge relayer's node stops here and a human decides. The
	// offending side chain stays stored (not marked bad), so a restart with
	...
	// fork choice (forkchoice.go) abandons for the authority chain however
	// deep the switch is.
	if maxDepth, depth := bc.cacheConfig.FerminuxMaxReorgDepth, len(oldChain); maxDepth > 0 && depth > maxDepth && bc.chainConfig.IsPosa(oldChain[0].Number()) {
		var newHead common.Hash
		if len(newChain) > 0 {
			newHead = newChain[0].Hash()
		}
		log.Error("REFUSING DEEP CHAIN REORG: depth exceeds the Ferminux safety cap; restart with --ferminux.allowdeepreorg ONLY after operator review",
			"depth", depth, "cap", maxDepth, "common", commonBlock.Number(), "commonhash", commonBlock.Hash(),
			"currenthead", bc.CurrentBlock().NumberU64(), "currenthash", bc.CurrentBlock().Hash(), "newhead", newHead, "dropfrom", oldChain[0].Hash())
		return fmt.Errorf("refusing chain reorg of depth %d (> Ferminux cap %d) from common block %d; restart with --ferminux.allowdeepreorg to accept",
			depth, maxDepth, commonBlock.NumberU64())
	}
