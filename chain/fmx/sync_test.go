...
func TestSnapSyncDisabling66(t *testing.T) { testSnapSyncDisabling(t, fmxproto.ETH66, snap.SNAP1) }
func TestSnapSyncDisabling67(t *testing.T) { testSnapSyncDisabling(t, fmxproto.ETH67, snap.SNAP1) }
...
func testSnapSyncDisabling(t *testing.T, ethVer uint, snapVer uint) {
	...
	emptyPeerEth := fmxproto.NewPeer(ethVer, p2p.NewPeer(enode.ID{1}, "", caps), emptyPipeEth, empty.txpool)
	fullPeerEth := fmxproto.NewPeer(ethVer, p2p.NewPeer(enode.ID{2}, "", caps), fullPipeEth, full.txpool)
	defer emptyPeerEth.Close()
	defer fullPeerEth.Close()

	go empty.handler.runEthPeer(emptyPeerEth, func(peer *fmxproto.Peer) error {
		return fmxproto.Handle((*ethHandler)(empty.handler), peer)
	})
	go full.handler.runEthPeer(fullPeerEth, func(peer *fmxproto.Peer) error {
		return fmxproto.Handle((*ethHandler)(full.handler), peer)
	})
