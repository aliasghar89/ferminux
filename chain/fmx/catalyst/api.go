...
func Register(stack *node.Node, backend *fmx.Ferminux) error {
...
type ConsensusAPI struct {
	...
	//     persist any bad block information to disk as a bug in Ferminux could end
	//     up blocking a valid chain, even if a later Ferminux update would accept
	//     it.
	//   - Bad blocks will get forgotten after a certain threshold of import
	//     attempts and will be retried. The rationale is that if the network
	//     really-really-really tries to feed us a block, we should give it a
	//     new chance, perhaps us being racey instead of the block being legit
	//     bad (this happened in Ferminux at a point with import vs. pending race).
	...
	// Ferminux can appear to be stuck or do strange things if the beacon client is
...
func NewConsensusAPI(eth *fmx.Ferminux) *ConsensusAPI {
