...
var (
	...
	// testAddr is the Ferminux address of the tester account.
...
func startEthService(t *testing.T, genesis *core.Genesis, blocks []*types.Block) (*node.Node, *fmx.Ferminux) {
	t.Helper()
	...
	ethcfg := &fmxconfig.Config{Genesis: genesis, Powhash: powhash.Config{PowMode: powhash.ModeFake}, SyncMode: downloader.FullSync, TrieTimeout: time.Minute, TrieDirtyCache: 256, TrieCleanCache: 256}
	ethservice, err := fmx.New(n, ethcfg)
	if err != nil {
...
func setupBlocks(t *testing.T, ethservice *fmx.Ferminux, n int, parent *types.Block, callback func(parent *types.Block)) {
	api := NewConsensusAPI(ethservice)
	for i := 0; i < n; i++ {
		callback(parent)
