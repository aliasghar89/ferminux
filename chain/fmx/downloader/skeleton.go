...
// the process. This is usually the case when Ferminux is shutting down and some events
...
//
// Opposed to the original Ferminux block synchronization which is trustless (and
...
// not from within (vs. legacy Ferminux sync).
//
...
type skeleton struct {
	db     fmxdb.Database // Database backing the skeleton
...
func newSkeleton(db fmxdb.Database, peers *peerSet, drop peerDropFn, filler backfiller) *skeleton {
	sk := &skeleton{
		db:         db,
		filler:     filler,
		peers:      peers,
...
func (s *skeleton) startup() {
	...
	for {
		...
		case errc := <-s.terminate:
			// No head was announced but Ferminux is shutting down
			errc <- nil
			return
		...
		case event := <-s.headEvents:
			...
			for {
				...
				default:
					...
					// error. Abort and wait until Ferminux requests a termination.
					errc := <-s.terminate
					errc <- err
					return
				}
...
func (s *skeleton) saveSyncStatus(db fmxdb.KeyValueWriter) {
...
func (s *skeleton) assignTasks(success chan *headerResponse, fail chan *headerRequest, cancel chan struct{}) {
	...
	for _, peer := range s.idles {
		...
		idlers.caps = append(idlers.caps, s.peers.rates.Capacity(peer.id, fmxproto.BlockHeadersMsg, targetTTL))
	}
...
func (s *skeleton) executeTask(peer *peerConnection, req *headerRequest) {
	...
	case <-timeoutTimer.C:
		...
		s.peers.rates.Update(peer.id, fmxproto.BlockHeadersMsg, 0, 0)
		s.scheduleRevertRequest(req)
	...
	case res := <-resCh:
		...
		headers := *res.Res.(*fmxproto.BlockHeadersPacket)

		headerReqTimer.Update(time.Since(start))
		s.peers.rates.Update(peer.id, fmxproto.BlockHeadersMsg, res.Time, len(headers))
