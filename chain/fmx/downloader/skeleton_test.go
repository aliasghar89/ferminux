...
func (p *skeletonTestPeer) RequestHeadersByNumber(origin uint64, amount int, skip int, reverse bool, sink chan *fmxproto.Response) (*fmxproto.Request, error) {
...
func (p *skeletonTestPeer) RequestHeadersByHash(common.Hash, int, int, bool, chan *fmxproto.Response) (*fmxproto.Request, error) {
	panic("skeleton sync must not request headers by hash")
}

func (p *skeletonTestPeer) RequestBodies([]common.Hash, chan *fmxproto.Response) (*fmxproto.Request, error) {
	panic("skeleton sync must not request block bodies")
}

func (p *skeletonTestPeer) RequestReceipts([]common.Hash, chan *fmxproto.Response) (*fmxproto.Request, error) {
...
func TestSkeletonSyncRetrievals(t *testing.T) {
	...
	for i, tt := range tests {
		...
		for _, peer := range tt.peers {
			peerset.Register(newPeerConnection(peer.id, fmxproto.ETH66, peer, log.New("id", peer.id)))
		}
		...
		if tt.newPeer != nil {
			if err := peerset.Register(newPeerConnection(tt.newPeer.id, fmxproto.ETH66, tt.newPeer, log.New("id", tt.newPeer.id))); err != nil {
