...
func TestTraceCall(t *testing.T) {
	...
	var testSuite = []struct {
		...
		},
		{
			...
			config: &TraceCallConfig{
				BlockOverrides: &fmxapi.BlockOverrides{Number: (*hexutil.Big)(big.NewInt(0x1337))},
			},
			expectErr: nil,
	...
	var testSuite = []struct {
		...
		// Call which can only succeed if state is state overridden
		{
			...
			config: &TraceCallConfig{
				StateOverrides: &fmxapi.StateOverride{
					randomAccounts[0].addr: fmxapi.OverrideAccount{Balance: newRPCBalance(new(big.Int).Mul(big.NewInt(1), big.NewInt(params.Ether)))},
				},
			},
		...
		//  }
		{
			...
			config: &TraceCallConfig{
				//Tracer: &tracer,
				StateOverrides: &fmxapi.StateOverride{
					randomAccounts[2].addr: fmxapi.OverrideAccount{
