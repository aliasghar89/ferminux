...
import (
	...
	"github.com/aliasghar89/ferminux/chain/fmx/fetcher"
...
// support all the operations needed by the Ferminux chain protocols.
...
type handlerConfig struct {
	Database       fmxdb.Database            // Database for direct sync insertions
...
// newHandler returns a handler for all Ferminux chain management protocol.
...
func (h *handler) runEthPeer(peer *fmxproto.Peer, handler fmxproto.Handler) error {
	...
	// Execute the Ferminux handshake
	var (
	...
	if err := peer.Handshake(h.networkID, td, hash, genesis.Hash(), forkID, h.forkFilter); err != nil {
		peer.Log().Debug("Ferminux handshake failed", "err", err)
		return err
	}
	...
	peer.Log().Debug("Ferminux peer connected", "name", peer.Name())

	// Register the peer locally
	if err := h.peers.registerPeer(peer, snap); err != nil {
		peer.Log().Error("Ferminux peer registration failed", "err", err)
		return err
	}
	...
	if h.checkpointHash != (common.Hash{}) {
		...
		resCh := make(chan *fmxproto.Response)
		...
		go func() {
			...
			select {
			case res := <-resCh:
				headers := ([]*types.Header)(*res.Res.(*fmxproto.BlockHeadersPacket))
				if len(headers) == 0 {
	...
	for number, hash := range h.requiredBlocks {
		...
		go func(number uint64, hash common.Hash, req *fmxproto.Request) {
			// Ensure the request gets cancelled in case of error/drop
			defer req.Close()
...
func (h *handler) unregisterPeer(id string) {
	...
	peer := h.peers.peer(id)
	if peer == nil {
		logger.Error("Ferminux peer removal failed", "err", errPeerNotRegistered)
		return
	}
	// Remove the `eth` peer if it exists
	logger.Debug("Removing Ferminux peer", "snap", peer.snapExt != nil)
	...
	if err := h.peers.unregisterPeer(id); err != nil {
		logger.Error("Ferminux peer removal failed", "err", err)
	}
}
...
func (h *handler) Stop() {
	...
	log.Info("Ferminux protocol stopped")
}
