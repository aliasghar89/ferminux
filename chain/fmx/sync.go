...
func (h *handler) syncTransactions(p *fmxproto.Peer) {
...
// chain head.
func (cs *chainSyncer) handlePeerEvent(peer *fmxproto.Peer) bool {
	select {
...
func peerToSyncOp(mode downloader.SyncMode, p *fmxproto.Peer) *chainSyncOp {
...
func (h *handler) doSync(op *chainSyncOp) error {
	if op.mode == downloader.SnapSync {
		...
		// txlookup limit.
		// The main concern here is: during the snap sync Ferminux won't index the
		// block(generate tx indices) before the HEAD-limit. But if user changes
		// the limit in the next snap sync(e.g. user kill Ferminux manually and
		// restart) then it will be hard for Ferminux to figure out the oldest block
		// has been indexed. So here for the user-experience wise, it's non-optimal
		// that user can't change limit during the snap sync. If changed, Ferminux
