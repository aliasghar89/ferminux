...
type headerRequesterFn func(common.Hash, chan *fmxproto.Response) (*fmxproto.Request, error)

// bodyRequesterFn is a callback type for sending a body retrieval request.
type bodyRequesterFn func([]common.Hash, chan *fmxproto.Response) (*fmxproto.Request, error)
				...
				go func(peer string) {
					...
					for _, hash := range hashes {
						...
						go func(hash common.Hash) {
							...
							case res := <-resCh:
								res.Done <- nil
								f.FilterHeaders(peer, *res.Res.(*fmxproto.BlockHeadersPacket), time.Now().Add(res.Time))
			...
			for peer, hashes := range request {
				...
				go func(peer string, hashes []common.Hash) {
					...
					case res := <-resCh:
						...
						txs, uncles := res.Res.(*fmxproto.BlockBodiesPacket).Unpack()
