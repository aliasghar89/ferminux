...
func makeChain(n int, seed byte, parent *types.Block) ([]common.Hash, map[common.Hash]*types.Block) {
	blocks, _ := core.GenerateChain(params.TestChainConfig, parent, powhash.NewFaker(), testdb, n, func(i int, block *core.BlockGen) {
...
func (f *fetcherTester) makeHeaderFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) headerRequesterFn {
	...
	return func(hash common.Hash, sink chan *fmxproto.Response) (*fmxproto.Request, error) {
...
func (f *fetcherTester) makeBodyFetcher(peer string, blocks map[common.Hash]*types.Block, drift time.Duration) bodyRequesterFn {
	...
	return func(hashes []common.Hash, sink chan *fmxproto.Response) (*fmxproto.Request, error) {
		...
		// Return on a new thread
		bodies := make([]*fmxproto.BlockBody, len(transactions))
		for i, txs := range transactions {
			bodies[i] = &fmxproto.BlockBody{
				Transactions: txs,
				Uncles:       uncles[i],
			}
		}
...
func testConcurrentAnnouncements(t *testing.T, light bool) {
	...
	counter := uint32(0)
	firstHeaderWrapper := func(hash common.Hash, sink chan *fmxproto.Response) (*fmxproto.Request, error) {
		atomic.AddUint32(&counter, 1)
		return firstHeaderFetcher(hash, sink)
	}
	secondHeaderWrapper := func(hash common.Hash, sink chan *fmxproto.Response) (*fmxproto.Request, error) {
		atomic.AddUint32(&counter, 1)
...
func testPendingDeduplication(t *testing.T, light bool) {
	...
	counter := uint32(0)
	headerWrapper := func(hash common.Hash, sink chan *fmxproto.Response) (*fmxproto.Request, error) {
		atomic.AddUint32(&counter, 1)

		// Simulate a long running fetch
		resink := make(chan *fmxproto.Response)
