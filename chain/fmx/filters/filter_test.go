...
func BenchmarkFilters(b *testing.B) {
	...
	var (
		...
		addr3   = common.BytesToAddress([]byte("ferminux"))
	...
	chain, receipts := core.GenerateChain(params.TestChainConfig, genesis, powhash.NewFaker(), db, 100010, func(i int, gen *core.BlockGen) {
		switch i {
		case 2403:
...
func TestFilters(t *testing.T) {
	...
	chain, receipts := core.GenerateChain(params.TestChainConfig, genesis, powhash.NewFaker(), db, 1000, func(i int, gen *core.BlockGen) {
		switch i {
		case 1:
			receipt := types.NewReceipt(nil, false, 0)
			receipt.Logs = []*types.Log{
