...
// information related to the Ferminux protocol such als blocks, transactions and logs.
...
func (api *FilterAPI) Logs(ctx context.Context, crit FilterCriteria) (*rpc.Subscription, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ferminux.FilterQuery(crit), matchedLogs)
	if err != nil {
		return nil, err
	}
...
// Same as ferminux.FilterQuery but with UnmarshalJSON() method.
type FilterCriteria ferminux.FilterQuery
...
func (api *FilterAPI) NewFilter(crit FilterCriteria) (rpc.ID, error) {
	...
	logsSub, err := api.events.SubscribeLogs(ferminux.FilterQuery(crit), logs)
	if err != nil {
		return "", err
	}
