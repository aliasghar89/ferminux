...
// testBackend is a mock implementation of the live Ferminux message handler. Its
...
func newTestBackendWithGenerator(blocks int, generator func(int, *core.BlockGen)) *testBackend {
	...
	chain, _ := core.NewBlockChain(db, nil, params.TestChainConfig, powhash.NewFaker(), vm.Config{}, nil, nil)

	bs, _ := core.GenerateChain(params.TestChainConfig, chain.Genesis(), powhash.NewFaker(), db, blocks, generator)
