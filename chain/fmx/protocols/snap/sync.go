...
type accountTask struct {
	...
	genBatch fmxdb.Batch     // Batch used by the node generator
...
// Syncer is a Ferminux account and storage trie syncer based on snapshots and
...
type Syncer struct {
	db fmxdb.KeyValueStore // Database to store the trie nodes into (and dedup)
	...
	stateWriter        fmxdb.Batch        // Shared batch writer used for persisting raw states
...
// NewSyncer creates a new snapshot syncer to download the Ferminux state over the
// snap protocol.
func NewSyncer(db fmxdb.KeyValueStore) *Syncer {
	return &Syncer{
		db: db,
...
func (s *Syncer) loadSyncStatus() {
	...
	if status := rawdb.ReadSnapshotSyncStatus(s.db); status != nil {
		if err := json.Unmarshal(status, &progress); err != nil {
			...
			for _, task := range s.tasks {
				task.genBatch = fmxdb.HookedBatch{
					Batch: s.db.NewBatch(),
				...
				for accountHash, subtasks := range task.SubTasks {
					for _, subtask := range subtasks {
						subtask.genBatch = fmxdb.HookedBatch{
							Batch: s.db.NewBatch(),
							OnPut: func(key []byte, value []byte) {
...
func (s *Syncer) processStorageResponse(res *storageResponse) {
	...
	if res.subTask != nil {
		...
		if res.subTask.genBatch.ValueSize() > fmxdb.IdealBatchSize || res.subTask.done {
...
func (s *Syncer) forwardAccountTask(task *accountTask) {
	...
	if task.genBatch.ValueSize() > fmxdb.IdealBatchSize || task.done {
		if err := task.genBatch.Write(); err != nil {
...
func (s *Syncer) onHealState(paths [][]byte, value []byte) error {
	...
	if s.stateWriter.ValueSize() > fmxdb.IdealBatchSize {
