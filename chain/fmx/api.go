...
// FerminuxAPI provides an API to access Ferminux full node-related information.
type FerminuxAPI struct {
	e *Ferminux
}

// NewFerminuxAPI creates a new Ferminux protocol API for full nodes.
func NewFerminuxAPI(e *Ferminux) *FerminuxAPI {
	return &FerminuxAPI{e}
}

// Etherbase is the address that mining rewards will be send to.
func (api *FerminuxAPI) Etherbase() (common.Address, error) {
	return api.e.Etherbase()
}

// Coinbase is the address that mining rewards will be send to (alias for Etherbase).
func (api *FerminuxAPI) Coinbase() (common.Address, error) {
	return api.Etherbase()
}

// Hashrate returns the POW hashrate.
func (api *FerminuxAPI) Hashrate() hexutil.Uint64 {
	return hexutil.Uint64(api.e.Miner().Hashrate())
}

// Mining returns an indication if this node is currently mining.
func (api *FerminuxAPI) Mining() bool {
...
func NewMinerAPI(e *Ferminux) *MinerAPI {
	return &MinerAPI{e}
}
...
// AdminAPI is the collection of Ferminux full node related APIs for node
// administration.
...
func NewAdminAPI(eth *Ferminux) *AdminAPI {
...
// DebugAPI is the collection of Ferminux full node APIs for debugging the
// protocol.
...
func NewDebugAPI(eth *Ferminux) *DebugAPI {
...
func (api *DebugAPI) GetBadBlocks(ctx context.Context) ([]*BadBlockArgs, error) {
	...
	for _, block := range blocks {
		...
		if blockJSON, err = fmxapi.RPCMarshalBlock(block, true, true, api.eth.APIBackend.ChainConfig()); err != nil {
