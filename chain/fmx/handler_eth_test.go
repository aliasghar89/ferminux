...
func (h *testEthHandler) TxPool() fmxproto.TxPool                   { panic("no backing tx pool") }
func (h *testEthHandler) AcceptTxs() bool                      { return true }
func (h *testEthHandler) RunPeer(*fmxproto.Peer, fmxproto.Handler) error { panic("not used in tests") }
func (h *testEthHandler) PeerInfo(enode.ID) interface{}        { panic("not used in tests") }

func (h *testEthHandler) Handle(peer *fmxproto.Peer, packet fmxproto.Packet) error {
	switch packet := packet.(type) {
	case *fmxproto.NewBlockPacket:
		h.blockBroadcasts.Send(packet.Block)
		return nil

	case *fmxproto.NewPooledTransactionHashesPacket:
		h.txAnnounces.Send(([]common.Hash)(*packet))
		return nil

	case *fmxproto.TransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil

	case *fmxproto.PooledTransactionsPacket:
		h.txBroadcasts.Send(([]*types.Transaction)(*packet))
		return nil
...
func TestForkIDSplit66(t *testing.T) { testForkIDSplit(t, fmxproto.ETH66) }
...
func testForkIDSplit(t *testing.T, protocol uint) {
	...
	peerNoFork := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pNoFork), p2pNoFork, nil)
	peerProFork := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pProFork), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()

	errc := make(chan error, 2)
	go func(errc chan error) {
		errc <- ethNoFork.runEthPeer(peerProFork, func(peer *fmxproto.Peer) error { return nil })
	}(errc)
	go func(errc chan error) {
		errc <- ethProFork.runEthPeer(peerNoFork, func(peer *fmxproto.Peer) error { return nil })
	}(errc)
	...
	peerNoFork = fmxproto.NewPeer(protocol, p2p.NewPeer(enode.ID{1}, "", nil), p2pNoFork, nil)
	peerProFork = fmxproto.NewPeer(protocol, p2p.NewPeer(enode.ID{2}, "", nil), p2pProFork, nil)
	defer peerNoFork.Close()
	defer peerProFork.Close()
...
func TestRecvTransactions66(t *testing.T) { testRecvTransactions(t, fmxproto.ETH66) }
...
func testRecvTransactions(t *testing.T, protocol uint) {
	...
	src := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, handler.txpool)
	sink := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, handler.txpool)
	defer src.Close()
	defer sink.Close()

	go handler.handler.runEthPeer(sink, func(peer *fmxproto.Peer) error {
		return fmxproto.Handle((*ethHandler)(handler.handler), peer)
	})
...
func TestSendTransactions66(t *testing.T) { testSendTransactions(t, fmxproto.ETH66) }
...
func testSendTransactions(t *testing.T, protocol uint) {
	...
	go handler.handler.runEthPeer(src, func(peer *fmxproto.Peer) error {
		return fmxproto.Handle((*ethHandler)(handler.handler), peer)
	})
	// Run the handshake locally to avoid spinning up a source handler
	var (
	...
	go fmxproto.Handle(backend, sink)
...
func TestTransactionPropagation66(t *testing.T) { testTransactionPropagation(t, fmxproto.ETH66) }
...
func testTransactionPropagation(t *testing.T, protocol uint) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{byte(i + 1)}, "", nil, sourcePipe), sourcePipe, source.txpool)
		sinkPeer := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, sink.txpool)
		defer sourcePeer.Close()
		defer sinkPeer.Close()

		go source.handler.runEthPeer(sourcePeer, func(peer *fmxproto.Peer) error {
			return fmxproto.Handle((*ethHandler)(source.handler), peer)
		})
		go sink.handler.runEthPeer(sinkPeer, func(peer *fmxproto.Peer) error {
			return fmxproto.Handle((*ethHandler)(sink.handler), peer)
		})
	}
...
func testCheckpointChallenge(t *testing.T, syncmode downloader.SyncMode, checkpoint bool, timeout bool, empty bool, match bool, drop bool) {
	...
	local := fmxproto.NewPeer(fmxproto.ETH66, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pLocal), p2pLocal, handler.txpool)
	remote := fmxproto.NewPeer(fmxproto.ETH66, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pRemote), p2pRemote, handler.txpool)
	defer local.Close()
	defer remote.Close()
	...
	go func() {
		...
		handler.handler.runEthPeer(local, func(peer *fmxproto.Peer) error {
			return fmxproto.Handle((*ethHandler)(handler.handler), peer)
		})
	}()
	...
	if checkpoint {
		...
		request := new(fmxproto.GetBlockHeadersPacket66)
...
func testBroadcastBlock(t *testing.T, peers, bcasts int) {
	...
	for i, sink := range sinks {
		...
		sourcePeer := fmxproto.NewPeer(fmxproto.ETH66, p2p.NewPeerPipe(enode.ID{byte(i)}, "", nil, sourcePipe), sourcePipe, nil)
		sinkPeer := fmxproto.NewPeer(fmxproto.ETH66, p2p.NewPeerPipe(enode.ID{0}, "", nil, sinkPipe), sinkPipe, nil)
		defer sourcePeer.Close()
		defer sinkPeer.Close()
		...
		go fmxproto.Handle(sink, sinkPeer)
	}
	// Subscribe to all the transaction pools
...
func TestBroadcastMalformedBlock66(t *testing.T) { testBroadcastMalformedBlock(t, fmxproto.ETH66) }
...
func testBroadcastMalformedBlock(t *testing.T, protocol uint) {
	...
	src := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{1}, "", nil, p2pSrc), p2pSrc, source.txpool)
	sink := fmxproto.NewPeer(protocol, p2p.NewPeerPipe(enode.ID{2}, "", nil, p2pSink), p2pSink, source.txpool)
	defer src.Close()
	defer sink.Close()

	go source.handler.runEthPeer(src, func(peer *fmxproto.Peer) error {
		return fmxproto.Handle((*ethHandler)(source.handler), peer)
	})
