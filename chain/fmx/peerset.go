...
func (ps *peerSet) registerSnapExtension(peer *snap.Peer) error {
	...
	if !peer.RunningCap(fmxproto.ProtocolName, fmxproto.ProtocolVersions) {
...
// by the peerset.
func (ps *peerSet) waitSnapExtension(peer *fmxproto.Peer) (*snap.Peer, error) {
...
func (ps *peerSet) registerPeer(peer *fmxproto.Peer, ext *snap.Peer) error {
...
func (ps *peerSet) peerWithHighestTD() *fmxproto.Peer {
	ps.lock.RLock()
	defer ps.lock.RUnlock()

	var (
		bestPeer *fmxproto.Peer
		bestTd   *big.Int
	)
	for _, p := range ps.peers {
