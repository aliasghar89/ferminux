...
The `faucet` is a simplistic web application with the goal of distributing small amounts of FMX in private and test networks.

Users need to post their Ferminux addresses to fund in a Twitter status update or public Facebook post and share the link to the faucet. The faucet will in turn deduplicate user requests and send the FMX. After a funding round, the faucet prevents the same user requesting again for a pre-configured amount of time, proportional to the amount of FMX requested.

## Operation

The `faucet` is a single binary app (everything included) with all configurations set via command line flags and a few files.

First thing's first, the `faucet` needs to connect to a Ferminux network, for which it needs the necessary genesis and network infos. Each of the following flags must be set:
...
The `faucet` will use the `les` protocol to join the configured Ferminux network and will store its data in `$HOME/.faucet` (currently not configurable).

## Funding

To be able to distribute funds, the `faucet` needs access to an already funded Ferminux account. This can be configured via:

- `-account.json` is a path to the Ferminux account's JSON key file
- `-account.pass` is a path to a text file with the decryption passphrase

The faucet is able to distribute various amounts of FMX in exchange for various timeouts. These can be configured via:

- `-faucet.amount` is the amount of FMX to send by default
