...
// faucet is an FMX faucet backed by a light client.
package main
...
var (
	...
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the Ferminux protocol")
	statsFlag   = flag.String("ethstats", "", "Ethstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Amount of FMX to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for Ferminux and the faucet")
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s FMX", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
...
type request struct {
	...
	Account common.Address     `json:"account"` // Ferminux address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by a Ferminux light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // Ferminux protocol stack
	client *fmxclient.Client   // Client connection to the Ferminux chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*enode.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the Ferminux light client protocol
	cfg := fmxconfig.Defaults
	...
	if err != nil {
		return nil, fmt.Errorf("Failed to register the Ferminux service: %w", err)
	}

	// Assemble the ethstats monitoring and reporting service'
	if stats != "" {
		if err := fmxstats.New(stack, lesBackend.ApiBackend, lesBackend.Engine(), stats); err != nil {
			return nil, err
		}
	}
	...
	client := fmxclient.NewClient(api)
...
// close terminates the Ferminux connection and tears down the faucet.
...
// apiHandler handles requests for FMX grants and transaction statuses.
func (f *faucet) apiHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		// Retrieve the Ferminux address to fund, the requesting user and a profile picture
		var (
			id       string
			username string
			avatar   string
		...
		default:
			//lint:ignore ST1005 This error is to be displayed in the browser
			err = errors.New("Something funky happened, please open an issue at https://github.com/aliasghar89/ferminux/issues")
		}
		if err != nil {
			if err = sendError(wsconn, err); err != nil {
...
// the uniqueness identifier (user id/username), username, avatar URL and Ferminux address to fund on success.
func authTwitter(url string, tokenV1, tokenV2 string) (string, string, string, common.Address, error) {
	...
	// and scrape it for the Ferminux address and profile URL. We need to load
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", "", common.Address{}, errors.New("No Ferminux address found to fund")
	}
	var avatar string
...
// API, returning the user id, username, avatar URL and Ferminux address to fund on
// success.
...
// returning the username, avatar URL and Ferminux address to fund on success.
func authFacebook(url string) (string, string, common.Address, error) {
	...
	// scrape it for the Ferminux address and profile URL.
	//
	...
	if address == (common.Address{}) {
		//lint:ignore ST1005 This error is to be displayed in the browser
		return "", "", common.Address{}, errors.New("No Ferminux address found to fund. Please check the post URL and verify that it can be viewed publicly.")
	}
	var avatar string
...
// authNoAuth tries to interpret a faucet request as a plain Ferminux address,
