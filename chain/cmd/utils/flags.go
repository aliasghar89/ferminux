...
var (
	...
	// Ferminux settings
	FerminuxAllowDeepReorgFlag = &cli.BoolFlag{
		Name:     "ferminux.allowdeepreorg",
		Usage:    "Allow chain reorganisations deeper than the Ferminux safety cap (64 blocks while the head is an authority block; inert under proof-of-work). DANGEROUS: operator-coordinated recovery only",
		Category: flags.EthCategory,
	}
...
func SetEthConfig(ctx *cli.Context, stack *node.Node, cfg *fmxconfig.Config) {
	...
	if ctx.IsSet(FerminuxAllowDeepReorgFlag.Name) {
		cfg.FerminuxAllowDeepReorg = ctx.Bool(FerminuxAllowDeepReorgFlag.Name)
	}
...
func MakeChain(ctx *cli.Context, stack *node.Node) (chain *core.BlockChain, chainDb fmxdb.Database) {
	...
	if ctx.Bool(FerminuxAllowDeepReorgFlag.Name) {
		cache.FerminuxMaxReorgDepth = 0
		log.Warn("Ferminux reorg-depth cap DISABLED by --ferminux.allowdeepreorg")
	}
