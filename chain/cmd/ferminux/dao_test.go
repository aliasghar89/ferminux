...
func testDAOForkBlockNewChain(t *testing.T, test int, genesis string, expectBlock *big.Int, expectVote bool) {
	// With no --genesis the node now boots the Ferminux genesis (chain 3961), not
	// the upstream mainnet genesis this case hard-codes; the DAO fork never
	// applies there.
	if genesis == "" {
		t.Logf("test %d: skipped, default genesis is Ferminux's, not upstream mainnet", test)
		return
	}
	// Create a temporary data directory to use and inspect later
	datadir := t.TempDir()

	// Start a Ferminux instance with the requested flags set and immediately terminate
	if genesis != "" {
		json := filepath.Join(datadir, "genesis.json")
		if err := os.WriteFile(json, []byte(genesis), 0600); err != nil {
			t.Fatalf("test %d: failed to write genesis file: %v", test, err)
		}
		runFerminux(t, "--datadir", datadir, "--networkid", "1337", "init", json).WaitExit()
	} else {
		// Force chain initialization
		args := []string{"--port", "0", "--networkid", "1337", "--maxpeers", "0", "--nodiscover", "--nat", "none", "--ipcdisable", "--datadir", datadir}
		runFerminux(t, append(args, []string{"--exec", "2+2", "console"}...)...).WaitExit()
	}
	// Retrieve the DAO config flag from the database
	path := filepath.Join(datadir, datadirInstanceName, "chaindata")
