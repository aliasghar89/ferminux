	...
	check := func(version string) {
		...
		for _, vuln := range vulns {
			...
			if vuln.Name == "Denial of service due to Go CVE-2020-28362" {
				// this one is not tied to ferminux-versions
				continue
			}
...
func TestFerminuxPubKeysParseable(t *testing.T) {
	for _, pubkey := range ferminuxPubKeys {
...
func TestKeyID(t *testing.T) {
	...
	tests := []struct {
		...
		{"@holiman key", args{id: extractKeyId(ferminuxPubKeys[0])}, "FB1D084D39BAEC24"},
		{"second key", args{id: extractKeyId(ferminuxPubKeys[1])}, "138B1CA303E51687"},
		{"third key", args{id: extractKeyId(ferminuxPubKeys[2])}, "FD9813B2D2098484"},
	}
