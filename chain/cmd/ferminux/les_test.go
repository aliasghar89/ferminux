...
type ferminuxrpc struct {
	name     string
	rpc      *rpc.Client
	ferminux     *testferminux
	nodeInfo *p2p.NodeInfo
}

func (g *ferminuxrpc) killAndWait() {
	g.ferminux.Kill()
	g.ferminux.WaitExit()
}

func (g *ferminuxrpc) callRPC(result interface{}, method string, args ...interface{}) {
	if err := g.rpc.Call(&result, method, args...); err != nil {
		g.ferminux.Fatalf("callRPC %v: %v", method, err)
	}
}

func (g *ferminuxrpc) addPeer(peer *ferminuxrpc) {
	g.ferminux.Logf("%v.addPeer(%v)", g.name, peer.name)
	enode := peer.getNodeInfo().Enode
	peerCh := make(chan *p2p.PeerEvent)
	sub, err := g.rpc.Subscribe(context.Background(), "admin", peerCh, "peerEvents")
	if err != nil {
		g.ferminux.Fatalf("subscribe %v: %v", g.name, err)
	}
	...
	select {
	case ev := <-peerCh:
		g.ferminux.Logf("%v received event: type=%v, peer=%v", g.name, ev.Type, ev.Peer)
	case err := <-sub.Err():
		g.ferminux.Fatalf("%v sub error: %v", g.name, err)
	case <-timeout:
		g.ferminux.Error("timeout adding peer after", dur)
	}
}

// Use this function instead of `g.nodeInfo` directly
func (g *ferminuxrpc) getNodeInfo() *p2p.NodeInfo {
	if g.nodeInfo != nil {
		return g.nodeInfo
	}
...
func startFerminuxWithIpc(t *testing.T, name string, args ...string) *ferminuxrpc {
	ipcName := fmt.Sprintf("ferminux-%d.ipc", atomic.AddUint32(&nextIPC, 1))
	args = append([]string{"--networkid=42", "--port=0", "--authrpc.port", "0", "--ipcpath", ipcName}, args...)
	t.Logf("Starting %v with rpc: %v", name, args)

	g := &ferminuxrpc{
		name: name,
		ferminux: runFerminux(t, args...),
	}
	ipcpath := ipcEndpoint(ipcName, g.ferminux.Datadir)
	// We can't know exactly how long ferminux will take to start, so we try 10
...
func initFerminux(t *testing.T) string {
	args := []string{"--networkid=42", "init", "./testdata/clique.json"}
	t.Logf("Initializing ferminux: %v ", args)
	g := runFerminux(t, args...)
	datadir := g.Datadir
	g.WaitExit()
	return datadir
}

func startLightServer(t *testing.T) *ferminuxrpc {
	datadir := initFerminux(t)
	t.Logf("Importing keys to ferminux")
	runFerminux(t, "account", "import", "--datadir", datadir, "--password", "./testdata/password.txt", "--lightkdf", "./testdata/key.prv").WaitExit()
	account := "0x02f0d131f1f97aef08aec6e3291b957d9efe7105"
	server := startFerminuxWithIpc(t, "lightserver", "--allow-insecure-unlock", "--datadir", datadir, "--password", "./testdata/password.txt", "--unlock", account, "--mine", "--light.serve=100", "--light.maxpeers=1", "--nodiscover", "--nat=extip:127.0.0.1", "--verbosity=4")
	return server
}

func startClient(t *testing.T, name string) *ferminuxrpc {
	datadir := initFerminux(t)
	return startFerminuxWithIpc(t, name, "--datadir", datadir, "--nodiscover", "--syncmode=light", "--nat=extip:127.0.0.1", "--verbosity=4")
}
