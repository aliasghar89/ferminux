...
func TestAccountListEmpty(t *testing.T) {
	ferminux := runFerminux(t, "account", "list")
	ferminux.ExpectExit()
}
...
`
	...
	{
		ferminux := runFerminux(t, "account", "list", "--datadir", datadir)
		ferminux.Expect(want)
		ferminux.ExpectExit()
	}
	{
		ferminux := runFerminux(t, "--datadir", datadir, "account", "list")
		ferminux.Expect(want)
		ferminux.ExpectExit()
	}
}
...
func TestAccountNew(t *testing.T) {
	ferminux := runFerminux(t, "account", "new", "--lightkdf")
	defer ferminux.ExpectExit()
	ferminux.Expect(`
...
func TestAccountHelp(t *testing.T) {
	ferminux := runFerminux(t, "account", "-h")
	ferminux.WaitExit()
	if have, want := ferminux.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}

	ferminux = runFerminux(t, "account", "import", "-h")
	ferminux.WaitExit()
	if have, want := ferminux.ExitStatus(), 0; have != want {
		t.Errorf("exit error, have %d want %d", have, want)
	}
...
func importAccountWithExpect(t *testing.T, key string, expected string) {
	...
	ferminux := runFerminux(t, "--lightkdf", "account", "import", "-password", passwordFile, keyfile)
	defer ferminux.ExpectExit()
	ferminux.Expect(expected)
}
...
func TestAccountUpdate(t *testing.T) {
	datadir := tmpDatadirWithKeystore(t)
	ferminux := runFerminux(t, "account", "update",
...
func TestWalletImport(t *testing.T) {
	ferminux := runFerminux(t, "wallet", "import", "--lightkdf", "testdata/guswallet.json")
	defer ferminux.ExpectExit()
	ferminux.Expect(`
!! Unsupported terminal, password will be echoed.
...
`)
	...
	files, err := os.ReadDir(filepath.Join(ferminux.Datadir, "keystore"))
	if len(files) != 1 {
...
func TestUnlockFlag(t *testing.T) {
	ferminux := runMinimalFerminux(t, "--port", "0", "--ipcdisable", "--datadir", tmpDatadirWithKeystore(t),
...
`)
	...
	for _, m := range wantMessages {
		if !strings.Contains(ferminux.StderrText(), m) {
...
func TestUnlockFlagAmbiguous(t *testing.T) {
	...
	ferminux.SetTemplateFunc("keypath", func(file string) string {
