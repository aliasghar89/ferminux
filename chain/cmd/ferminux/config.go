...
import (
	...
	"github.com/aliasghar89/ferminux/chain/accounts/usbwallet"
...
type ferminuxConfig struct {
	Eth      fmxconfig.Config
	Node     node.Config
	Ethstats ethstatsConfig
	Metrics  metrics.Config
}

func loadConfig(file string, cfg *ferminuxConfig) error {
	f, err := os.Open(file)
	if err != nil {
		return err
	}
...
func defaultNodeConfig() node.Config {
	...
	// NOT clientIdentifier: Config.Name is the on-disk instance directory and
	// the nodekey location, frozen independently of the product/binary name.
	cfg.Name = datadirInstanceName
...
// makeConfigNode loads ferminux configuration and creates a blank node instance.
func makeConfigNode(ctx *cli.Context) (*node.Node, ferminuxConfig) {
	// Load defaults.
	cfg := ferminuxConfig{
		Eth:     fmxconfig.Defaults,
...
// makeFullNode loads ferminux configuration and creates the Ferminux backend.
func makeFullNode(ctx *cli.Context) (*node.Node, fmxapi.Backend) {
	...
	if eth != nil && !ctx.IsSet(utils.IgnoreLegacyReceiptsFlag.Name) {
		...
		if err != nil {
			...
			stack.Close()
			utils.Fatalf("Database has receipts with a legacy format. Please run `ferminux db freezer-migrate`.")
		}
	}
	...
	// Add the Ferminux Stats daemon if requested.
	if cfg.Ethstats.URL != "" {
...
func applyMetricConfig(ctx *cli.Context, cfg *ferminuxConfig) {
...
func deprecated(field string) bool {
	switch field {
	case "fmxconfig.Config.EVMInterpreter":
		return true
	case "fmxconfig.Config.EWASMInterpreter":
		return true
	default:
		return false
	}
