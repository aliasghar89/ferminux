...
func showLeveldbStats(db fmxdb.KeyValueStater) {
...
type snapshotIterator struct {
	init    bool
	account fmxdb.Iterator
	storage fmxdb.Iterator
}
...
var chainExporters = map[string]func(db fmxdb.Database) utils.ChainDataIterator{
	"preimage": func(db fmxdb.Database) utils.ChainDataIterator {
		iter := db.NewIterator(rawdb.PreimagePrefix, nil)
		return &preimageIterator{iter: iter}
	},
	"snapshot": func(db fmxdb.Database) utils.ChainDataIterator {
...
func dbHasLegacyReceipts(db fmxdb.Database, firstIdx uint64) (bool, uint64, error) {
	// Check first block for legacy receipt format
	numAncients, err := db.Ancients()
	if err != nil {
		return false, 0, err
