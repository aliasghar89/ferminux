...
Use `devp2p enrdump <base64>` to verify and display a Ferminux Node Record.
...
1. initialize the ferminux node with the `genesis.json` file contained in the `testdata` directory
2. import the `halfchain.rlp` file in the `testdata` directory
3. run ferminux with the following flags:
```
ferminux --datadir <datadir> --nodiscover --nat=none --networkid 19763 --verbosity 5
```

Then, run the following command, replacing `<enode>` with the enode of the ferminux node:
 ```
...
To run the eth66 protocol test suite, initialize a ferminux node as described above and run the following command,
replacing `<enode>` with the enode of the ferminux node:
