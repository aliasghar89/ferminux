...
func (c *Conn) readAndServe(chain *Chain, timeout time.Duration) Message {
	...
	for time.Since(start) < timeout {
		...
		case *GetBlockHeaders:
			...
			resp := &BlockHeaders{
				RequestId:          msg.ReqID(),
				BlockHeadersPacket: fmxproto.BlockHeadersPacket(headers),
			}
...
func (s *Suite) waitForBlockImport(conn *Conn, block *types.Block) error {
	...
	// create request
	req := &GetBlockHeaders{
		GetBlockHeadersPacket: &fmxproto.GetBlockHeadersPacket{
			Origin: fmxproto.HashOrNumber{Hash: block.Hash()},
			Amount: 1,
		},
	}
...
func (s *Suite) hashAnnounce() error {
	...
	err = sendConn.Write(&BlockHeaders{
		...
		BlockHeadersPacket: fmxproto.BlockHeadersPacket{nextBlock.Header()},
	})
	if err != nil {
		return fmt.Errorf("failed to write to connection: %v", err)
	}
