...
```text
$ clef --keystore ~/.ferminux/rinkeby/keystore --chainid 4

INFO [07-01|11:00:46.385] Starting signer                          chainid=4 keystore=$HOME/.ferminux/rinkeby/keystore light-kdf=false advanced=false
...
You can select which accounts the caller can see
  [x] 0xD9C9Cd5f6779558b6e0eD4e6Acf6b1947E7fA1F3
    URL: keystore://$HOME/.ferminux/rinkeby/keystore/UTC--2017-04-14T15-15-00.327614556Z--d9c9cd5f6779558b6e0ed4e6acf6b1947e7fa1f3
  [x] 0x086278A6C067775F71d6B2BB1856Db6E28c30418
    URL: keystore://$HOME/.ferminux/rinkeby/keystore/UTC--2018-02-06T22-53-11.211657239Z--086278a6c067775f71d6b2bb1856db6e28c30418
-------------------------------------------
Request context:
	NA -> NA -> NA
...
```text
$ clef --keystore ~/.ferminux/rinkeby/keystore --chainid 4 --rules rules.js

INFO [07-01|13:39:49.726] Rule engine configured                   file=rules.js
INFO [07-01|13:39:49.726] Starting signer                          chainid=4 keystore=$HOME/.ferminux/rinkeby/keystore light-kdf=false advanced=false
...
*Note, to make this example work, please use you own accounts. You can create a new account either via Clef or the traditional account CLI tools. If the latter was chosen, make sure both Clef and Ferminux use the same keystore by specifying `--keystore path/to/your/keystore` when running Clef.*
...
INFO [07-01|14:12:41.636] Starting signer                          chainid=4 keystore=$HOME/.ferminux/rinkeby/keystore light-kdf=false advanced=false
...
## Ferminux integration

Of course, as awesome as Clef is, it's not feasible to interact with it via JSON RPC by hand. Long term, we're hoping to convince the general Ferminux community to support Clef as a general signer (it's only 3-5 methods), thus allowing your favorite DApp, Metamask, MyCrypto, etc to request signatures directly.

Until then however, we're trying to pave the way via Ferminux. Ferminux v1.9.0 has built in support via `--signer <API endpoint>` for using a local or remote Clef instance as an account backend!

We can try this by running Clef with our previous rules on Rinkeby (for now it's a good idea to allow auto-listing accounts, since Ferminux likes to retrieve them once in a while).

```text
$ clef --keystore ~/.ferminux/rinkeby/keystore --chainid 4 --rules rules.js
```

In a different window we can start Ferminux, list our accounts, even list our wallets to see where the accounts originate from:

```text
$ ferminux --rinkeby --signer=~/.clef/clef.ipc console
...
*Note, if you enable the external signer backend in Ferminux, all other account management is disabled. This is because long term we want to remove account management from Ferminux.*
