...
// Verify that Client implements the ferminux interfaces.
var (
	_ = ferminux.ChainReader(&Client{})
	_ = ferminux.TransactionReader(&Client{})
	_ = ferminux.ChainStateReader(&Client{})
	_ = ferminux.ChainSyncReader(&Client{})
	_ = ferminux.ContractCaller(&Client{})
	_ = ferminux.GasEstimator(&Client{})
	_ = ferminux.GasPricer(&Client{})
	_ = ferminux.LogFilterer(&Client{})
	_ = ferminux.PendingStateReader(&Client{})
	// _ = ferminux.PendingStateEventer(&Client{})
	_ = ferminux.PendingContractCaller(&Client{})
)
...
func newTestBackend(t *testing.T) (*node.Node, []*types.Block) {
	...
	// Create Ferminux Service
	config := &fmxconfig.Config{Genesis: genesis}
	config.Powhash.PowMode = powhash.ModeFake
	ethservice, err := fmx.New(n, config)
	if err != nil {
		t.Fatalf("can't create new ferminux service: %v", err)
	}
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == ferminux.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != ferminux.NotFound {
		t.Fatal("error should be ferminux.NotFound")
	}
}
...
func testChainID(t *testing.T, client *rpc.Client) {
	...
	if id == nil || id.Cmp(params.AllPowhashProtocolChanges.ChainID) != 0 {
