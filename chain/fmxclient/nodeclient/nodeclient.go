...
// Package nodeclient provides an RPC client for ferminux-specific APIs.
package nodeclient
...
// Client is a wrapper around rpc.Client that implements ferminux-specific functionality.
//
// If you want to use the standardized Ferminux RPC functionality, use fmxclient.Client instead.
type Client struct {
	c *rpc.Client
}
...
func (ec *Client) CreateAccessList(ctx context.Context, msg ferminux.CallMsg) (*types.AccessList, uint64, string, error) {
...
// the message call.
// Please use fmxclient.CallContract instead if you don't need the override functionality.
func (ec *Client) CallContract(ctx context.Context, msg ferminux.CallMsg, blockNumber *big.Int, overrides *map[common.Address]OverrideAccount) ([]byte, error) {
...
// GCStats retrieves the current garbage collection stats from a ferminux node.
...
// MemStats retrieves the current memory stats from a ferminux node.
...
// GetNodeInfo retrieves the node info of a ferminux node.
