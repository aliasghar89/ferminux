...
// Tests that powhash works correctly in test mode.
func TestTestMode(t *testing.T) {
	header := &types.Header{Number: big.NewInt(1), Difficulty: big.NewInt(100)}

	powhash := NewTester(nil, false)
	defer powhash.Close()

	results := make(chan *types.Block)
	err := powhash.Seal(nil, types.NewBlockWithHeader(header), results, nil)
	if err != nil {
	...
	case block := <-results:
		...
		if err := powhash.verifySeal(nil, header, false); err != nil {
...
func verifyTest(wg *sync.WaitGroup, e *Powhash, workerIndex, epochs int) {
	defer wg.Done()
...
func TestRemoteSealer(t *testing.T) {
	...
	sealhash := powhash.SealHash(header)
...
func TestHashrate(t *testing.T) {
	...
	if tot := powhash.Hashrate(); tot != float64(expect) {
...
func TestClosedRemoteSealer(t *testing.T) {
	...
	api := &API{powhash}
	if _, err := api.GetWork(); err != errPowhashStopped {
