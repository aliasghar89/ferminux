...
func TestRemoteNotify(t *testing.T) {
	...
	// Create the custom powhash engine.
	powhash := NewTester([]string{server.URL}, false)
	defer powhash.Close()
	...
	powhash.Seal(nil, block, nil, nil)
	select {
	case work := <-sink:
		if want := powhash.SealHash(header).Hex(); work[0] != want {
...
func TestRemoteNotifyFull(t *testing.T) {
	...
	powhash := New(config, []string{server.URL}, false)
	defer powhash.Close()
...
func TestRemoteMultiNotify(t *testing.T) {
	...
	// Create the custom powhash engine.
	powhash := NewTester([]string{server.URL}, false)
	powhash.config.Log = testlog.Logger(t, log.LvlWarn)
	defer powhash.Close()
	...
	for i := 0; i < cap(sink); i++ {
		...
		block := types.NewBlockWithHeader(header)
		powhash.Seal(nil, block, results, nil)
	}
...
func TestStaleSubmission(t *testing.T) {
	...
	for id, c := range testcases {
		for _, h := range c.headers {
			powhash.Seal(nil, types.NewBlockWithHeader(h), results, nil)
		}
		if res := api.SubmitWork(fakeNonce, powhash.SealHash(c.headers[c.submitIndex]), fakeDigest); res != c.submitRes {
