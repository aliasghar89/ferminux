...
// BreakGlassMessage returns the raw EIP-191 message the Ferminux multisig
// owners must personal_sign to authorise `signers` at block `number` built on
// `parentHash` (normally the current head).
func (api *API) BreakGlassMessage(number uint64, parentHash common.Hash, signers []common.Address) (hexutil.Bytes, error) {
	msg, err := api.clique.BreakGlassMessage(number, parentHash, signers)
	if err != nil {
		return nil, err
	}
	return hexutil.Bytes(msg), nil
}

// BreakGlass validates the owner signatures and arms the override for the
// block this node will seal at `number` on top of `parentHash` (see
// ferminux.go). Keep the clique namespace on IPC only: arming needs the owner
// signatures, but DiscardBreakGlass does not.
func (api *API) BreakGlass(number uint64, parentHash common.Hash, signers []common.Address, signatures []hexutil.Bytes) (bool, error) {
	sigs := make([][]byte, len(signatures))
	for i, sig := range signatures {
		sigs[i] = sig
	}
	if err := api.clique.ArmBreakGlass(number, parentHash, signers, sigs); err != nil {
		return false, err
	}
	return true, nil
}

// BreakGlassStatus returns the armed override, if any.
func (api *API) BreakGlassStatus() *BreakGlassOverride {
	return api.clique.BreakGlassPending()
}

// DiscardBreakGlass drops an armed override.
func (api *API) DiscardBreakGlass() {
	api.clique.DisarmBreakGlass()
}
