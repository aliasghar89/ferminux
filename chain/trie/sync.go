...
type Sync struct {
	database fmxdb.KeyValueReader         // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database fmxdb.KeyValueReader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) Commit(dbw fmxdb.Batch) error {
