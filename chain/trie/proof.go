...
func (t *Trie) Prove(key []byte, fromLevel uint, proofDb fmxdb.KeyValueWriter) error {
...
// nodes of the longest existing prefix of the key (at least the root node), ending
// with the node that proves the absence of the key.
func (t *StateTrie) Prove(key []byte, fromLevel uint, proofDb fmxdb.KeyValueWriter) error {
...
func VerifyProof(rootHash common.Hash, key []byte, proofDb fmxdb.KeyValueReader) (value []byte, err error) {
	key = keybytesToHex(key)
	wantHash := rootHash
	for i := 0; ; i++ {
...
func proofToPath(rootHash common.Hash, root node, key []byte, proofDb fmxdb.KeyValueReader, allowNonExistent bool) (node, []byte, error) {
...
func VerifyRangeProof(rootHash common.Hash, firstKey []byte, lastKey []byte, keys [][]byte, values [][]byte, proof fmxdb.KeyValueReader) (bool, error) {
