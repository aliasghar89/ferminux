...
// Package remotedb implements the key-value database layer based on a remote ferminux
// node. Under the hood, it utilises the `debug_dbGet` method to implement a
// read-only database.
// There really are no guarantees in this database, since the local ferminux does not
...
func (db *Database) ReadAncients(fn func(op fmxdb.AncientReaderOp) error) (err error) {
	return fn(db)
}
...
func (db *Database) ModifyAncients(f func(fmxdb.AncientWriteOp) error) (int64, error) {
	panic("not supported")
}
...
func (db *Database) NewBatch() fmxdb.Batch {
	panic("not supported")
}

func (db *Database) NewBatchWithSize(size int) fmxdb.Batch {
	panic("not supported")
}

func (db *Database) NewIterator(prefix []byte, start []byte) fmxdb.Iterator {
	panic("not supported")
}
...
func (db *Database) NewSnapshot() (fmxdb.Snapshot, error) {
	panic("not supported")
}
...
func New(endpoint string) (fmxdb.Database, error) {
