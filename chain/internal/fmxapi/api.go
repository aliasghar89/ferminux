...
import (
	...
	"github.com/aliasghar89/ferminux/chain/accounts/scwallet"
...
// FerminuxAPI provides an API to access Ferminux related information.
type FerminuxAPI struct {
	b Backend
}

// NewFerminuxAPI creates a new Ferminux protocol API.
func NewFerminuxAPI(b Backend) *FerminuxAPI {
	return &FerminuxAPI{b}
}

// GasPrice returns a suggestion for a gas price for legacy transactions.
func (s *FerminuxAPI) GasPrice(ctx context.Context) (*hexutil.Big, error) {
...
func (s *FerminuxAPI) MaxPriorityFeePerGas(ctx context.Context) (*hexutil.Big, error) {
	tipcap, err := s.b.SuggestGasTipCap(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *FerminuxAPI) FeeHistory(ctx context.Context, blockCount rpc.DecimalOrHex, lastBlock rpc.BlockNumber, rewardPercentiles []float64) (*feeHistoryResult, error) {
...
func (s *FerminuxAPI) Syncing() (interface{}, error) {
...
// FerminuxAccountAPI provides an API to access accounts managed by this node.
// It offers only methods that can retrieve accounts.
type FerminuxAccountAPI struct {
	am *accounts.Manager
}

// NewFerminuxAccountAPI creates a new FerminuxAccountAPI.
func NewFerminuxAccountAPI(am *accounts.Manager) *FerminuxAccountAPI {
	return &FerminuxAccountAPI{am: am}
}

// Accounts returns the collection of accounts this node manages.
func (s *FerminuxAccountAPI) Accounts() []common.Address {
...
// Sign calculates a Ferminux ECDSA signature for:
...
// Unpair deletes a pairing between wallet and ferminux.
...
// BlockChainAPI provides an API to access Ferminux blockchain data.
type BlockChainAPI struct {
	b Backend
}

// NewBlockChainAPI creates a new Ferminux blockchain API.
func NewBlockChainAPI(b Backend) *BlockChainAPI {
	return &BlockChainAPI{b}
}

// ChainId is the EIP-155 replay-protection chain id for the current Ferminux chain config.
//
...
// DebugAPI is the collection of Ferminux APIs exposed over the debugging
// namespace.
...
func (api *DebugAPI) SeedHash(ctx context.Context, number uint64) (string, error) {
	...
	return fmt.Sprintf("%#x", powhash.SeedHash(number)), nil
}
...
// Version returns the current ferminux protocol version.
