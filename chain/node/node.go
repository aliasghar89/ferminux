...
func New(conf *Config) (*Node, error) {
	...
	// Creates an empty AccountManager with no backends. Callers (e.g. cmd/ferminux)
...
func (n *Node) OpenDatabase(name string, cache, handles int, namespace string, readonly bool) (fmxdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) OpenDatabaseWithFreezer(name string, cache, handles int, ancient string, namespace string, readonly bool) (fmxdb.Database, error) {
	n.lock.Lock()
	defer n.lock.Unlock()
	if n.state == closedState {
		return nil, ErrNodeStopped
...
func (n *Node) wrapDatabase(db fmxdb.Database) fmxdb.Database {
