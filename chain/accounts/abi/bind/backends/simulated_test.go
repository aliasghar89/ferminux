...
func TestSimulatedBackend(t *testing.T) {
	...
	if err != ferminux.NotFound {
		t.Fatalf("err should be `ferminux.NotFound` but received %v", err)
	}
...
func TestNewSimulatedBackend(t *testing.T) {
	...
	if sim.config != params.AllPowhashProtocolChanges {
		t.Errorf("expected sim config to equal params.AllEthashProtocolChanges, got %v", sim.config)
	}

	if sim.blockchain.Config() != params.AllPowhashProtocolChanges {
...
func TestEstimateGas(t *testing.T) {
	...
	var cases = []struct {
		name        string
		message     ferminux.CallMsg
		expect      uint64
		expectError error
		expectData  interface{}
	}{
		{"plain transfer(valid)", ferminux.CallMsg{
			From:     addr,
			To:       &addr,
			Gas:      0,
		...
		{"plain transfer(invalid)", ferminux.CallMsg{
			From:     addr,
			To:       &contractAddr,
			Gas:      0,
			GasPrice: big.NewInt(0),
...
func TestEstimateGasWithPrice(t *testing.T) {
	...
	var cases = []struct {
		...
		{"EstimateWithoutPrice", ferminux.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
			GasPrice: big.NewInt(0),
		...
		{"EstimateWithPrice", ferminux.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithVeryHighPrice", ferminux.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateWithSuperhighPrice", ferminux.CallMsg{
			From:     addr,
			To:       &recipient,
			Gas:      0,
		...
		{"EstimateEIP1559WithHighFees", ferminux.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
		...
		{"EstimateEIP1559WithSuperHighFees", ferminux.CallMsg{
			From:      addr,
			To:        &addr,
			Gas:       0,
			GasFeeCap: big.NewInt(1e14), // maxgascost = 2.1ether
...
func TestPendingAndCallContract(t *testing.T) {
	...
	res, err := sim.PendingCallContract(bgCtx, ferminux.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	})
	...
	res, err = sim.CallContract(bgCtx, ferminux.CallMsg{
		From: testAddr,
		To:   &addr,
		Data: input,
	}, nil)
...
func TestCallContractRevert(t *testing.T) {
	...
	call[0] = func(input []byte) ([]byte, error) {
		return sim.PendingCallContract(bgCtx, ferminux.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		})
	...
	call[1] = func(input []byte) ([]byte, error) {
		return sim.CallContract(bgCtx, ferminux.CallMsg{
			From: testAddr,
			To:   &addr,
			Data: input,
		}, nil)
