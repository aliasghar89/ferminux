...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and Ferminux address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs a Ferminux transaction after having the user validate the parameters
	ledgerOpGetConfiguration ledgerOpcode = 0x06 // Returns specific wallet application configuration
	ledgerOpSignTypedMessage ledgerOpcode = 0x0c // Signs a Ferminux message following the EIP 712 specification
...
// offline returns whether the wallet and the Ferminux app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// Ferminux app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the Ferminux app's version, will fail prior to v1.0.2
...
// and returning the Ferminux address located on that derivation path.
...
//
// Note, if the version of the Ferminux application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonetheless, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the Ferminux app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the Ferminux wallet app running
...
// ledgerDerive retrieves the currently active Ferminux address from a Ledger
...
//   Ferminux address length | 1 byte
//   Ferminux address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the Ferminux hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into a Ferminux address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Extract the Ferminux signature and do a sanity validation
