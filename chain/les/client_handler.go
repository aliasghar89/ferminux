...
func newClientHandler(ulcServers []string, ulcFraction int, checkpoint *params.TrustedCheckpoint, backend *LightFerminux) *clientHandler {
...
func (h *clientHandler) handle(p *serverPeer, noInitAnnounce bool) error {
	...
	p.Log().Debug("Light Ferminux peer connected", "name", p.Name())
	...
	if err := p.Handshake(h.backend.blockchain.Genesis().Hash(), forkid, h.forkFilter); err != nil {
		p.Log().Debug("Light Ferminux handshake failed", "err", err)
		return err
	}
	...
	if err := h.backend.peers.register(p); err != nil {
		p.Log().Error("Light Ferminux peer registration failed", "err", err)
		return err
	}
	...
	for {
		if err := h.handleMsg(p); err != nil {
			p.Log().Debug("Light Ferminux message handling failed", "err", err)
			p.fcServer.DumpLogs()
			return err
		}
	}
...
func (h *clientHandler) handleMsg(p *serverPeer) error {
	...
	p.Log().Trace("Light Ferminux message arrived", "code", msg.Code, "bytes", msg.Size)
...
func (d *downloaderPeerNotify) registerPeer(p *serverPeer) {
	...
	h.downloader.RegisterLightPeer(p.id, fmxproto.ETH66, pc)
}
