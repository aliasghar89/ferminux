...
import (
	...
	fmxdownloader "github.com/aliasghar89/ferminux/chain/fmx/downloader"
	...
	"github.com/aliasghar89/ferminux/chain/p2p/simulations"
	"github.com/aliasghar89/ferminux/chain/p2p/simulations/adapters"
...
func newLesClientService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	config := fmxconfig.Defaults
	config.SyncMode = (fmxdownloader.SyncMode)(downloader.LightSync)
...
func newLesServerService(ctx *adapters.ServiceContext, stack *node.Node) (node.Lifecycle, error) {
	config := fmxconfig.Defaults
	config.SyncMode = (fmxdownloader.SyncMode)(downloader.FullSync)
	config.LightServ = testServerCapacity
	config.LightPeers = testMaxClients
	ferminux, err := fmx.New(stack, &config)
	if err != nil {
		return nil, err
	}
	_, err = NewLesServer(stack, ferminux, &config)
	if err != nil {
		return nil, err
	}
	return ferminux, nil
