/* Copied from agents/web/src/motion.ts on 2026-09-24 (surfaces/explorer.md §1.1: liveText, calm, onPlay,
   initMotion; the reveal code is kept but unused, the explorer carries no [data-rv]). Explorer additions
   are at the end, below the copied file: the home-feed insert (§7.3). */
...
/** Explorer: register a [data-loop] element that a page created after boot (the shared observer only saw the
 *  first document). It plays within 200 px of the viewport; the page's `signal` unregisters it on navigation. */
export function observeLoop(el: HTMLElement, signal?: AbortSignal) {
  if (!lio) { el.dataset.play = "on"; return; }
  lio.observe(el);
  signal?.addEventListener("abort", () => {
    lio?.unobserve(el);
    for (let i = loops.length - 1; i >= 0; i--) if (loops[i][0] === el) loops.splice(i, 1);
  }, { once: true });
}
...
export function initMotion() {
  ...
  lio = "IntersectionObserver" in window ? new IntersectionObserver((es) => {
    for (const e of es) {
...
/**
 * Home feeds (§7.3): prepend real new rows to an <ol class="feed"> with a FLIP slide.
 *  1. the list jumps up by the new rows' height and slides back to 0 over 280 ms --ease-out;
 *  2. new rows fade 0 → 1 over 200 ms; only the newest one flashes (its ::after, 1200 ms, compositor-only);
 *  3. rows beyond `cap` fade out over 150 ms and are removed.
 * Calm tier: rows appear in place and carry a static 2 px accent bar for 2 s.
 * `rows` are the new <li> elements, newest first.
 */
export function feedInsert(list: HTMLElement | null, rows: HTMLElement[], cap: number) {
  if (!list || !rows.length) return;
  const quiet = calm();
  rows.slice().reverse().forEach((r) => list.prepend(r));
  const extra = Array.from(list.children).slice(cap) as HTMLElement[];
  if (quiet) {
    extra.forEach((r) => r.remove());
    rows.forEach((r) => { r.classList.add("fresh-calm"); window.setTimeout(() => r.classList.remove("fresh-calm"), 2000); });
    return;
  }
  const h = rows.reduce((s, r) => s + r.getBoundingClientRect().height, 0);
  list.animate([{ transform: `translateY(${-h}px)` }, { transform: "none" }], { duration: 280, easing: "cubic-bezier(.16,1,.3,1)" });
  rows.forEach((r) => r.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 200, easing: "cubic-bezier(.16,1,.3,1)", fill: "backwards" }));
  rows[0].classList.add("fresh");
  window.setTimeout(() => rows[0].classList.remove("fresh"), 1300);
  extra.forEach((r) => {
    const a = r.animate([{ opacity: 1 }, { opacity: 0 }], { duration: 150, easing: "cubic-bezier(.4,0,1,1)", fill: "forwards" });
    a.onfinish = () => r.remove();
  });
}

/** Run a DOM swap inside a view transition when the browser has one and motion is on (§7.6).
 *  Resolves once the swap has run (the transition calls it asynchronously), so focus can follow. */
export function withTransition(swap: () => void): Promise<void> {
  const d = document as Document & { startViewTransition?: (cb: () => void) => { updateCallbackDone: Promise<void> } };
  if (!calm() && typeof d.startViewTransition === "function" && !document.hidden) {
    try { return d.startViewTransition(swap).updateCallbackDone.catch(() => { /* the swap ran or threw; the router reports */ }); }
    catch { swap(); return Promise.resolve(); }
  }
  swap();
