 ...
 */
const OPS = ["balance", "block", "gas", "agent", "agents", "stats", "job", "validate", "help"] as const;
...
export const chainHandler: Handler = async (input) => {
  ...
  switch (op) {
    ...
    case "agents": { const r = await f.agents.list({ q: arg || undefined, status: "active" }); return { ok: true, op, agents: r.items ?? r }; }
    ...
    default: return { ok: true, op: "help", ops: OPS, examples: ["balance 0x…", "block", "gas", "agent 1", "agents translate", "job 1", "stats", '{"op":"validate","jobId":1,"requestHash":"0x…"}'] };
  }
};
