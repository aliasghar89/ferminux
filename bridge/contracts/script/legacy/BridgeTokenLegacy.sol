...
// =============================================================================
// THE WRAPPER THAT IS DEPLOYED ON BSC TODAY — a frozen copy, for rehearsal only.
//
// wFMX at 0x73e64635E2a7b393F2aa3924dcf91fE3cFF51BD0 runs this code: a burn()
// that the bridge may call against any balance, with no allowance consulted.
// src/BridgeToken.sol has since made burn() debit the holder's allowance, which
// changes the runtime bytecode, which changes the codehash the bridge pins —
// and route registration is once-only, so the live wrapper can never be
// replaced in place. Migrating to the new wrapper means redeploying BOTH
// bridges, and the unwind leg of that migration operates on THIS code.
//
// It is kept here so the migration rehearsal exercises the real thing rather
// than a stand-in that already behaves the way we are migrating toward. A
// rehearsal that unwinds the NEW wrapper would prove nothing about unwinding
// the one actually holding value.
//
// DO NOT DEPLOY THIS. It is a fixture. New wrappers come from src/BridgeToken.sol.
// Delete this file once the live wFMX is retired and the migration is done.
// =============================================================================
...
contract BridgeToken {
    ...
    /// @notice Destroy `amount` held by `from`. Only the bridge, only inside
    ///         send() — the caller of send() is always `from`, so this is never
    ///         a unilateral confiscation by the bridge operator: the bridge has
    ///         no entry point that burns anyone other than msg.sender.
