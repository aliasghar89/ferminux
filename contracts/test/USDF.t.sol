...
contract USDFTest is Test {
    ...
    function test_Metadata() public view {
        assertEq(aznt.name(), "Ferminux Dollar");
        assertEq(aznt.symbol(), "USDF");
    ...
    function test_GrantRole_OnlyAdmin() public {
        ...
        vm.prank(alice);
        vm.expectRevert(bytes("USDF: not admin"));
    ...
    function test_RevokeRole_ClearsAndEmits() public {
        ...
        vm.prank(minter);
        vm.expectRevert(bytes("USDF: missing role"));
        aznt.mint(alice, 1);
    }
    ...
    function test_AcceptAdmin_OnlyPending() public {
        ...
        aznt.transferAdmin(alice);
        vm.prank(bob);
        vm.expectRevert(bytes("USDF: not pending admin"));
        aznt.acceptAdmin();
    }
    ...
    function test_Mint_RevertsWhenPaused() public {
        ...
        aznt.pause();
        vm.prank(minter);
        vm.expectRevert(bytes("USDF: paused"));
        aznt.mint(alice, 1);
    }
    ...
    function test_Mint_RevertsToBlacklisted() public {
        ...
        vm.prank(minter);
        vm.expectRevert(bytes("USDF: blacklisted"));
        aznt.mint(alice, 1);
    }
    ...
    function test_DestroyBlackFunds_RevertsIfNotBlacklisted() public {
        vm.prank(blacklister);
        vm.expectRevert(bytes("USDF: not blacklisted"));
    ...
    function test_Transfer_RevertsToZeroAddress() public {
        _mint(alice, 100);
        vm.prank(alice);
        vm.expectRevert(bytes("USDF: transfer to zero"));
    ...
    function test_TransferFrom_RevertsOnInsufficientAllowance() public {
        ...
        aznt.approve(bob, 10);
        vm.prank(bob);
        vm.expectRevert(bytes("USDF: insufficient allowance"));
    ...
    function test_Permit_RevertsOnWrongSigner() public {
        ...
        vm.expectRevert(bytes("USDF: invalid signature"));
        aznt.permit(permitOwner, bob, 777, deadline, v, r, s);
    }
    ...
    function test_Permit_RevertsWhenExpired() public {
        ...
        (uint8 v, bytes32 r, bytes32 s) =
            vm.sign(ownerKey, _permitDigest(permitOwner, bob, 777, aznt.nonces(permitOwner), deadline));
        vm.expectRevert(bytes("USDF: permit expired"));
        aznt.permit(permitOwner, bob, 777, deadline, v, r, s);
    }
