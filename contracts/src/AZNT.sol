...
/**
 * @title AZNT — Ferminux Manat
 * @notice AZN (Azerbaijani manat) reserve-backed stablecoin on Ferminux Network.
 *         1 AZNT = 1 AZN, backed 1:1 by off-chain bank reserves.
...
contract AZNT {
    // ---------------------------------------------------------------- ERC20
    string public constant name = "Ferminux Manat";
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "AZNT: not admin");
        _;
    }

    modifier onlyRole(bytes32 role) {
        require(hasRole[role][msg.sender], "AZNT: missing role");
        _;
    }
    ...
    modifier notBlacklisted(address account) {
        require(!blacklisted[account], "AZNT: blacklisted");
        _;
    }
    ...
    constructor(address _admin) {
        require(_admin != address(0), "AZNT: zero admin");
        admin = _admin;
    ...
    function acceptAdmin() external {
        require(msg.sender == pendingAdmin, "AZNT: not pending admin");
    ...
    // ----------------------------------------------------------- Mint/Burn
    /// @notice Mint AZNT after a verified AZN bank deposit. Ops-side only.
    ...
    /// @notice Burn from the ops wallet when AZN is redeemed to the user's bank.
    ...
    function destroyBlackFunds(address account) external onlyRole(BLACKLISTER) {
        require(blacklisted[account], "AZNT: not blacklisted");
        ...
        returns (bool)
    {
        ...
        if (allowed != type(uint256).max) {
            require(allowed >= amount, "AZNT: insufficient allowance");
    ...
    function _transfer(address from, address to, uint256 amount) internal {
        require(to != address(0), "AZNT: transfer to zero");
    ...
    function permit(
        ...
        require(block.timestamp <= deadline, "AZNT: permit expired");
        ...
        require(recovered != address(0) && recovered == owner, "AZNT: invalid signature");
